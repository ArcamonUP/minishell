...
static void	exec_child(int fd, t_node *node, t_shell *data)
{
	...
	if (!cmd)
		(exec_free(data->tab, data->envp, node), exit(1));
	if (!cmd[0] || empty_line(cmd[0]))
		(free_tab(cmd), exec_free(data->tab, data->envp, node), exit(0));
	path = get_path(cmd[0], data->envp);
	if (!path)
	{
		(error("minishell: ", cmd[0]), error(": command not found\n", NULL));
		(free_tab(cmd), exec_free(data->tab, data->envp, node), exit(127));
	}
	...
	(error("minishell: ", cmd[0]), error(": command not found\n", NULL));
	(ft_lstfd_clear(&node->fdin), ft_lstfd_clear(&node->fdout), free(path));
	(free_tab(cmd), exec_free(data->tab, data->envp, node), exit(127));
}
